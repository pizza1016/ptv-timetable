...
class TramTrackerAPI(object):
    ...
    def call(self: Self, request: str) -> _responsemodel.TramTrackerResponse:
    ...
    def list_destinations(self: Self) -> list[TramDestination]:
        ...
        response = cast(_responsemodel.DestinationsResponse, self.call("/GetAllRoutes.ashx"))
    ...
    def list_stops(self: Self, route_id: int, up_direction: bool) -> list[TramStop]:
        ...
        response = cast(_responsemodel.StopsResponse, self.call(f"/GetStopsByRouteAndDirection.ashx?r={route_id}&u={"true" if up_direction else "false"}"))
    ...
    def get_stop(self: Self, stop_id: int) -> TramStop:
        ...
        response = cast(_responsemodel.StopResponse, self.call(f"/GetStopInformation.ashx?s={stop_id}"))
    ...
    def list_routes_for_stop(self: Self, stop_id: int) -> list[str]:
        ...
        response = cast(_responsemodel.RoutesResponse, self.call(f"/GetPassingRoutes.ashx?s={stop_id}"))
    ...
    def next_trams(self: Self, stop_id: int, route_id: int | None = None, low_floor_tram: bool = False, as_of: datetime = datetime.now(tz=ZoneInfo("Australia/Melbourne"))) -> list[TramDeparture]:
        ...
        response = cast(_responsemodel.DeparturesResponse, self.call(f"/GetNextPredictionsForStop.ashx?stopNo={stop_id}&routeNo={route_id if route_id is not None else 0}&isLowFloor={"true" if low_floor_tram else "false"}&ts={timestamp}"))
    ...
    def get_route_colour(self: Self, route_id: int, as_of: datetime = datetime.now(tz=TZ_MELBOURNE)) -> str:
        ...
        response = cast(_responsemodel.ColourResponse, self.call(f"/GetRouteColour.ashx?routeNo={route_id}&ts={timestamp}"))
    ...
    def get_route_text_colour(self: Self, route_id: int, as_of: datetime = datetime.now(tz=TZ_MELBOURNE)) -> str:
        ...
        if as_of.tzinfo is None:
            as_of = as_of.replace(tzinfo=TZ_MELBOURNE)
        timestamp = round((as_of - datetime(1970, 1, 1, tzinfo=timezone.utc)) / timedelta(milliseconds=1))
        response = cast(_responsemodel.ColourResponse, self.call(f"/GetRouteTextColour.ashx?routeNo={route_id}&ts={timestamp}"))
        return "#" + response["responseObject"]["Colour"].lower()
